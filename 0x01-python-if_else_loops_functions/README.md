...
In this project, I started using `if` statements, `if-else` statements, `break`,
`continue`, `pass`, and `range` statements with `while` and `for` loops in Python.
I also practiced creating my own Python functions and learned about variable scope,
tracebacks, and arithmetic operators.
